  ...
  switch (message.MsgType) {
    ...
    case 10000:
      ...
      if (helper.isChatRoom(userid)) {
        ...
        if (members.show && members.user.UserName === userid) {
...
class Chat {
  ...
  @action async sendEmojiMessage(auth, message) {
    var response = await axios.post(
      `/cgi-bin/mmwebwx-bin/webwxsendemoticon?fun=sys&lang=en_US&pass_ticket=${auth.passTicket}`,
      {
        BaseRequest: {
          Sid: auth.wxsid,
          Uin: auth.wxuin,
  ...
  @action async sendVideoMessage(auth, message, isForward) {
    var response = await axios.post(
      `/cgi-bin/mmwebwx-bin/webwxsendvideomsg?fun=async&f=json&pass_ticket=${auth.passTicket}`,
      {
        BaseRequest: {
          Sid: auth.wxsid,
          Uin: auth.wxuin,
  ...
  @action async sendMessage(user, message, isForward = false, transformMessages = self.transformMessages) {
    ...
    if (data.BaseResponse.Ret === 0) {
      ...
      if (!helper.isChatRoom(user.UserName) && !helper.isContact(user)) {
        // The target is not your friend
        list.data.push({
          Content: `${user.sex ? 'She' : 'He'} is not your friend, <a class="add-friend" data-userid="${user.UserName
